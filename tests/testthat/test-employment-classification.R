...
test_that("vecshift handles default overlap classification", {
  ...
  overlaps <- result[arco > 1]
  if (nrow(overlaps) > 0) {
    # With normalized prior values and vecshift event processing, overlap group
    # contains both ft and pt priors, producing a mixed label
...
test_that("vecshift status logic covers all possible combinations", {
  ...
  for (overlap_type in overlap_types) {
    ...
    overlaps <- result[arco > 1]
    expect_true(
      overlap_type %in% overlaps$stato,
