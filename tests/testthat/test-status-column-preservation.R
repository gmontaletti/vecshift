...
test_that("merge_consecutive_employment preserves stato column for all consolidation types", {
  ...
  vecshift_result <- classify_employment_status(vecshift_result)
  ...
  for (consolidation_type in consolidation_types) {
    ...
    expect_true(
      "stato" %in% names(result),
      info = paste(
        "stato column missing for consolidation_type:",
        consolidation_type
      )
    )
    expect_true(
      all(!is.na(result$stato)),
      info = paste(
        "NA values in stato for consolidation_type:",
        consolidation_type
      )
    )
  }
