...
test_that("process_employment_pipeline works with basic parameters", {
  ...
  expect_true(all(
    c("cf", "inizio", "fine", "arco", "durata") %in% names(result)
  ))
...
test_that("process_employment_pipeline validates input parameters", {
  ...
  expect_error(
    process_employment_pipeline(
      valid_data,
      merge_columns = "nonexistent_column"
    ),
...
test_that("get_pipeline_recommendations detects data quality issues", {
  ...
  problematic_data <- data.table(
    ...
    fine = as.Date(c(
      "2023-02-28",
      "2023-02-15",
      "2023-06-30", # Second has FINE < INIZIO
      "2023-08-31",
      "2023-10-31",
      "2023-12-31"
    )),
...
test_that("pipeline handles overlapping employment correctly", {
  ...
  if (nrow(overlap_segments) > 0) {
    ...
    expect_true(any(
      grepl("->", overlap_segments$company) | overlap_segments$salary == 80000
    ))
  }
})
...
test_that("pipeline memory usage and performance tracking", {
  ...
  medium_data[
    fine < inizio,
    fine := inizio + sample(1:30, sum(fine < inizio), replace = TRUE)
  ]
  ...
  expect_true(all(
    c("cf", "inizio", "fine", "arco", "company") %in% names(result)
  ))
})
...
test_that("pipeline handles empty and minimal datasets", {
  ...
  result_minimal <- process_employment_pipeline(
    minimal_data,
    collapse_consecutive = FALSE,
    show_progress = FALSE
  )
