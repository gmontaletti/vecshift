...
- original_data:
  ...
  A data.table containing the original contract records that were
  processed by vecshift(). Must include 'id' column and the columns
  specified in the 'columns' parameter.

- segments:

  A data.table containing the output from vecshift(). Must include 'id'
  column for merging.

- columns:

  Character vector specifying which columns from original_data to merge
  into segments. Column names must exist in original_data.

## Value

A data.table combining the segments data with the requested columns from
original_data. The temporal ordering and structure of segments is
...
original contract data using the 'id' column as the key. Since
vecshift() creates multiple temporal segments from single contracts, the
same original contract data may appear in multiple segment rows.
