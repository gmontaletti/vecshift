...
print("1. Processing through pipeline to get over_id...")

# Process through the full pipeline to preserve additional columns
...
if (has_over_id) {
  ...
  if ("tooltip" %in% names(g6_data_consolidated$nodes)) {
    ...
    print("- Enhanced tooltips not found (check implementation)")
  }
  
  if ("style" %in% names(g6_data_consolidated$edges)) {
    print("✓ Edge styling for consolidation available")
  } else {
    print("- Edge styling not found (check implementation)")
  }
  ...
  reduction_pct <- round((nrow(transitions_raw) - nrow(transitions_consolidated)) / nrow(transitions_raw) * 100, 1)
  if (reduction_pct > 0) {
    print(paste("• Consolidation reduced transitions by", reduction_pct, "%"))
  }
