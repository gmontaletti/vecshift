...
.perform_aggregation <- function(dt_work, numeric_extra, character_extra) {
  ...
  if ("stato" %in% names(dt_work)) {
    agg_list[["stato"]] <- quote(if(.N == 1 || !is_employment[1]) {
      stato[1]
    } else {
...
merge_consecutive_employment_fast <- function(dt) {
  ...
  # Include stato (employment status) in aggregation if it exists
  if ("stato" %in% names(dt_work)) {
    agg_exprs[["stato"]] <- quote(if(.N == 1 || employment_status[1] == 0) {
      stato[1]
    } else {
      # For collapsed employment periods, prioritize the most common status
      # or use the first status if all are different
...
merge_consecutive_employment_fast_with_type <- function(dt, consolidation_type = "both") {
  require("data.table")
  ...
  if (consolidation_type == "overlapping") {
    ...
    dt_work[, employment_status := ifelse(arco > 0, 1, 0)]
    dt_work[, consolidation_group := ifelse(employment_status == 1 & over_id > 0, 
                                           paste0(cf, "_", over_id), 
                                           paste0(cf, "_", .I))]  # Each unemployment period gets unique group
  } else {
    ...
    if (consolidation_type == "consecutive") {
      ...
      dt_work[, has_gap := employment_status == 1 & !is.na(prev_fine) & 
              (as.numeric(inizio - prev_fine) > 1)]
      dt_work[, consolidation_group := paste0(cf, "_", cumsum(has_gap | status_change))]
    } else {
      # "both" - use over_id for overlapping, then consecutive logic
      if (has_over_id) {
        # First consolidate by over_id for employment periods
        dt_work[, temp_group := ifelse(employment_status == 1 & over_id > 0, 
                                      paste0("emp_", over_id), 
                                      paste0("unemp_", cumsum(employment_status == 0 & 
                                        (is.na(shift(employment_status)) | shift(employment_status) == 1))))]
  ...
  # Identify extra columns
  base_cols <- c("cf", "inizio", "fine", "arco", "prior", "durata", "id", "stato", "over_id")
  internal_cols <- c("employment_status", "status_change", "group_id", "prev_fine", 
                    "has_gap", "consolidation_group")
