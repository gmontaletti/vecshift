...
#'
#' @export
#' @importFrom data.table data.table copy setDT setattr
#'
#' @examples
#' \dontrun{
#' library(data.table)
...
#' )
#' }
process_employment_pipeline <- function(
  original_data,
  apply_vecshift = TRUE,
  merge_columns = NULL,
  ...
  if (show_progress) {
    ...
    if (apply_vecshift) {
      ...
      if (classify_status) {
        ...
        progress_names <- c(
          progress_names,
          "Applying employment status classification"
        )
      }
  ...
  if (!inherits(original_data, "data.table")) {
    stop(
      "Parameter 'original_data' must be a data.table object. Use as.data.table() to convert."
    )
  }
  ...
  if (validate_functions) {
    if (
      !is.null(merge_columns) &&
        !exists("merge_original_columns", mode = "function")
    ) {
      stop(
        "Function 'merge_original_columns' not found. Please ensure the merge_columns.R file is loaded."
      )
    }
    ...
    if (
      !is.null(merge_columns) &&
        handle_overlaps &&
        !exists("merge_overlapping_values", mode = "function")
    ) {
      stop(
        "Function 'merge_overlapping_values' not found. Please ensure the merge_columns.R file is loaded."
      )
    }
    ...
    if (collapse_consecutive) {
      if (
        ...
        stop(
          "Function 'merge_consecutive_employment' not found. Please ensure the merge_consecutive_employment.R file is loaded."
        )
      } else if (
        !consolidate_periods &&
          !exists("merge_consecutive_employment_fast", mode = "function")
      ) {
        stop(
          "Function 'merge_consecutive_employment_fast' not found. Please ensure the merge_consecutive_employment.R file is loaded."
        )
      }
    }
    ...
    if (
      validate_over_id &&
        !exists("validate_over_id_consistency", mode = "function")
    ) {
      warning(
        "Function 'validate_over_id_consistency' not found. over_id validation will be skipped."
      )
  ...
  if (!is.null(merge_columns)) {
    if (!is.character(merge_columns) || length(merge_columns) == 0) {
      stop(
        "Parameter 'merge_columns' must be a non-empty character vector or NULL."
      )
    }
  ...
  if (apply_vecshift) {
    ...
    if (!exists("vecshift", mode = "function")) {
      stop(
        "Function 'vecshift' not found. Please ensure the vecshift.R file is loaded."
      )
    }
    ...
    if (classify_status) {
      ...
      if (exists("classify_employment_status", mode = "function")) {
        result <- classify_employment_status(
          result,
          rules = status_rules,
          group_by = "cf"
        )
        ...
        warning(
          "classify_employment_status function not found. Skipping status classification."
        )
      ...
      if (exists("validate_duration_invariant", mode = "function")) {
        ...
        if (getOption("vecshift.verbose", FALSE)) {
          ...
          if (!validation_results$duration$all_tests_passed) {
            cat(
              "Warning: Duration invariant validation found inconsistencies.\n"
            )
          }
        }
      }
  ...
  if (!is.null(merge_columns)) {
    ...
    if (exists("merge_original_columns", mode = "function")) {
      ...
      if (getOption("vecshift.verbose", FALSE)) {
        cat(
          "Step 2 (merge_original_columns) completed. Columns added:",
          length(merge_columns),
          "\n"
        )
      }
      ...
      if (
        handle_overlaps && exists("merge_overlapping_values", mode = "function")
      ) {
        if (show_progress) {
        ...
        warning(
          "merge_overlapping_values function not available. Skipping overlap handling."
        )
      }
    } else {
      warning(
        "merge_original_columns function not available. Skipping column merging step."
      )
    }
  }
  ...
  if (collapse_consecutive) {
    ...
    if (
      ...
      if ("collapsed" %in% names(result)) {
        consolidation_stats$periods_consolidated <- sum(
          result$collapsed,
          na.rm = TRUE
        )
        consolidation_stats$total_periods_after <- nrow(result)
        consolidation_stats$consolidation_type <- consolidation_type
        consolidation_stats$reduction_ratio <- 1 -
          (nrow(result) / pre_consolidation_rows)
      }

      if (getOption("vecshift.verbose", FALSE)) {
        cat(sprintf(
          "Step 3 (merge_consecutive_employment) completed. %s consolidation: %d -> %d rows.\n",
          consolidation_type,
          pre_consolidation_rows,
          nrow(result)
        ))
      }
    } else if (
      ...
      if (getOption("vecshift.verbose", FALSE)) {
        cat(
          "Step 3 (merge_consecutive_employment_fast) completed. Final rows:",
          nrow(result),
          "\n"
        )
      }
      ...
      warning(
        "Neither merge_consecutive_employment nor merge_consecutive_employment_fast function available. Skipping period consolidation."
      )
  ...
  if (nrow(result) == 0) {
    warning(
      "Pipeline resulted in empty data.table. Please check input data and parameters."
    )
  }
  ...
  if (show_progress) {
    ...
    summary_msg <- sprintf(
      "Pipeline completed in %.2f seconds. Processed %d rows -> %d rows.",
    ...
    if (length(validation_results) > 0) {
      ...
      if (
        !is.null(validation_results$consolidation) &&
          !is.null(validation_results$consolidation$periods_consolidated)
      ) {
...
get_pipeline_recommendations <- function(data, target_operation = "analysis") {
  ...
  if (!target_operation %in% valid_operations) {
    stop(paste(
      "Parameter 'target_operation' must be one of:",
  ...
  if (target_operation == "analysis") {
    ...
    reasoning <- c(
      reasoning,
      "Analysis target: enabling period consolidation with over_id for comprehensive temporal segments"
    )
    ...
    reasoning <- c(
      reasoning,
      "Reporting target: preserving individual periods for detailed reports"
    )
    ...
    reasoning <- c(
      reasoning,
      "Visualization target: consolidating periods using over_id for cleaner plots"
    )
    ...
    reasoning <- c(
      reasoning,
      "Export target: comprehensive processing with over_id consolidation for external use"
    )
  }
  ...
  if (n_rows > 100000) {
    warnings <- c(
      ...
      paste(
        ...
        n_rows,
        " rows). Consider processing in chunks for memory efficiency."
      )
    )
  }
  ...
  if ("inizio" %in% names(data) && "fine" %in% names(data)) {
    ...
    if (invalid_ranges > 0) {
      warnings <- c(
        ...
        paste(
          invalid_ranges,
          "records have fine < inizio. Consider data cleaning."
        )
      )
    }
  }
  ...
  if (!availability["merge_original_columns"] && !is.null(merge_columns)) {
    warnings <- c(
      warnings,
      "merge_original_columns not available. Column merging will be skipped."
    )
  ...
  if (!availability["merge_consecutive_employment"]) {
    if (!availability["merge_consecutive_employment_fast"]) {
      warnings <- c(
        warnings,
        "Neither merge_consecutive_employment nor merge_consecutive_employment_fast available. Period consolidation will be skipped."
      )
      recommendations$collapse_consecutive <- FALSE
      recommendations$consolidate_periods <- FALSE
    } else {
      warnings <- c(
        warnings,
        "merge_consecutive_employment not available. Falling back to merge_consecutive_employment_fast (no over_id support)."
      )
      recommendations$consolidate_periods <- FALSE
    }
  }
  ...
  if (!availability["validate_over_id_consistency"]) {
    warnings <- c(
      warnings,
      "validate_over_id_consistency not available. over_id validation will be skipped."
    )
