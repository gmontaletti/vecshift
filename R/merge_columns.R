...
merge_original_columns <- function(original_data, segments, columns) {
  ...
  if (!inherits(segments, "data.table")) {
    stop(
      "Parameter 'segments' must be a data.table object. Use as.data.table() to convert."
    )
  }

  if (!is.character(columns) || length(columns) == 0) {
    stop(
      "Parameter 'columns' must be a non-empty character vector specifying column names."
    )
  }
  ...
  if (length(conflicting_cols) > 0) {
    warning(paste(
      "Column name conflicts detected. Columns from original_data will overwrite segments columns:",
  ...
  if (nrow(employment_segments) > 0) {
    ...
    unmatched_ids <- unique(employment_segments$id[
      !employment_segments$id %in% merge_data$id
    ])
    if (length(unmatched_ids) > 0) {
      warning(paste(
        "Some employment segment IDs were not found in original data:",
  ...
  if (nrow(merged_employment) > 0 && nrow(unemployment_segments) > 0) {
    result <- rbindlist(
      list(merged_employment, unemployment_segments),
      use.names = TRUE,
      fill = TRUE
    )
  ...
  if (nrow(result) > 0) {
    if ("cf" %in% names(result) && "inizio" %in% names(result)) {
      ...
      warning(
        "Could not restore temporal ordering. Missing 'cf' or 'inizio' columns."
      )
    }
  }
...
merge_overlapping_values <- function(segments_with_columns, columns) {
  ...
  if (!inherits(segments_with_columns, "data.table")) {
    stop(
      "Parameter 'segments_with_columns' must be a data.table object. Use as.data.table() to convert."
    )
  }
  ...
  if ("inizio" %in% names(result)) {
    ...
    warning(
      "Column 'inizio' not found. Results may not be properly ordered by time."
    )
    setorder(result, cf)
  }
