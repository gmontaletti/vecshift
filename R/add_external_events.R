...
#' @importFrom data.table setnames setkeyv set setcolorder first as.IDate
#'
#' @examples
#' \dontrun{
#' library(data.table)
...
add_external_events <- function(
  ...
  if (length(missing_vecshift_cols) > 0) {
    stop(paste(
      ...
      paste(missing_vecshift_cols, collapse = ", ")
    ))
  }
  ...
  if (
    !is.numeric(synthetic_unemployment_duration) ||
      synthetic_unemployment_duration <= 0
  ) {
  ...
  estimated_memory_gb <- as.numeric(n_persons_with_events) *
    as.numeric(avg_unemployment_per_person) *
    50 *
    8 /
    (1024^3) # More realistic estimate

  if (!memory_safe && estimated_memory_gb > 4 && progress) {
    message(sprintf(
      "Large dataset detected (estimated %.1f GB intermediate data). Consider using memory_safe = TRUE",
      estimated_memory_gb
    ))
  }
  ...
  if (progress) {
    message(sprintf(
      "Processing %d vecshift records with %d external events",
      n_rows_vecshift,
      n_rows_events
    ))
  }
  ...
  if (create_synthetic_unemployment) {
    ...
    if (progress) {
      message("Creating synthetic unemployment periods...")
    }
  ...
  if (progress) {
    message(sprintf(
      "Initializing %d event attribute columns",
  ...
  if (event_matching_strategy == "overlap") {
    if (progress) {
      message("Starting overlap matching...")
    }
    ...
    if (progress) {
      message("Starting nearest neighbor matching...")
    }
...
#' @keywords internal
.create_synthetic_unemployment <- function(
  vecshift_data,
  events_data,
  person_id_column,
  max_duration,
  ...
  # Preserve the date class from input (Date or IDate)
  date_class_input <- class(vecshift_data$fine)[1]

  max_date_in_main <- max(vecshift_data$fine, na.rm = TRUE)

  # Also consider event dates to ensure synthetic unemployment covers them
  event_start_dates <- events_data[[date_columns["start"]]]
  event_end_dates <- if (
    "end" %in%
      names(date_columns) &&
      date_columns["end"] %in% names(events_data)
  ) {
    events_data[[date_columns["end"]]]
  } else {
    event_start_dates
  }
  ...
  reference_end_date <- max(c(max_date_in_main, max_event_date), na.rm = TRUE)
  ...
  # Preserve date class from input (Date or IDate)
  synthetic_periods[, `:=`(
...
.prepare_events_data <- function(
  ...
  # Keep date columns in their original class (Date or IDate)
  # No conversion needed - preserve what the user provided
...
#' @keywords internal
.match_events_overlap_optimized <- function(
  vecshift_data,
  events_data,
  event_name_column,
  person_id_column,
  ...
  if (progress) {
    message("Using fully vectorized foverlaps() for efficient event matching")
  }
  ...
  if (progress) {
    message(sprintf("Found %d overlaps from foverlaps", nrow(overlaps)))
  }
  ...
  if (nrow(unemployment_overlaps) > 0) {
    ...
    if (progress) {
      message(sprintf(
        "Processed %d direct unemployment overlaps",
        nrow(unemployment_overlaps)
      ))
    }
  }
  ...
  if (nrow(unemployment_overlaps) > 0) {
    events_without_unemployment <- events_data[
      !unemployment_overlaps,
      on = .(cf, event_start, event_end, event_name)
    ]
  } else {
  ...
  if (nrow(events_without_unemployment) > 0) {
    ...
    unemployment_periods <- vecshift_data[
      arco == 0,
      .(cf, inizio, fine, .row_id)
    ]
    ...
    unemployment_event_cross <- unemployment_periods[
      events_without_unemployment,
      on = "cf",
    ...
    if (nrow(preceding_candidates) > 0) {
      preceding_matches <- preceding_candidates[
        preceding_candidates[,
          .I[which.max(fine)],
          by = .(cf, event_start, event_end, event_name)
        ]$V1
      ]
    } else {
    ...
    if (nrow(valid_preceding) > 0) {
      ...
      # FIX: Wrap both operands in as.numeric to avoid IDate storage mode corruption
      valid_preceding[, distance := as.numeric(event_start) - as.numeric(fine)]
      ...
      for (event_name in unique_events_preceding) {
        ...
        set(
          vecshift_data,
          i = event_matches$.row_id,
          j = col_dist,
          value = event_matches$distance
        )
        set(
          vecshift_data,
          i = event_matches$.row_id,
      ...
      if (progress) {
        message(sprintf(
          "Processed %d preceding unemployment matches",
          nrow(valid_preceding)
        ))
      }
    }
    ...
    if (nrow(valid_preceding) > 0) {
      events_no_preceding <- events_without_unemployment[
        !valid_preceding,
        on = .(cf, event_start, event_end, event_name)
      ]
    } else {
    ...
    if (nrow(events_no_preceding) > 0) {
      ...
      events_with_first <- events_no_preceding[
        first_contracts,
        on = "cf",
        nomatch = 0L
      ]
      ...
      if (nrow(viable_synthetic) > 0) {
        ...
        synthetic_periods[,
          c("event_names", "earliest_event", "first_contract") := NULL
        ]
        ...
        if (progress) {
          message(sprintf(
            "Created %d synthetic unemployment periods",
            nrow(synthetic_periods)
          ))
        }
      }
    }
  ...
  if (progress && synthetic_rows_added > 0) {
    message(sprintf(
      "Added %d synthetic unemployment rows",
...
#' @keywords internal
.match_events_nearest_optimized <- function(
  vecshift_data,
  events_data,
  event_name_column,
  person_id_column,
  ...
  unemployment_periods[,
    midpoint := (as.numeric(inizio) + as.numeric(fine)) / 2
  ]

  # Calculate midpoints for events
  events_with_midpoint <- copy(events_data)
  events_with_midpoint[,
    midpoint := (as.numeric(event_start) + as.numeric(event_end)) / 2
  ]
  ...
  if (progress) {
    message(sprintf(
      "Processing %d persons with both events and unemployment periods",
      length(common_persons)
    ))
  }
  ...
  event_unemployment_combinations <- unemployment_filtered[
    events_filtered,
    on = "cf",
    allow.cartesian = TRUE
  ]
  ...
  if (progress) {
    message(sprintf(
      "Calculated %d event-unemployment distance combinations",
      nrow(event_unemployment_combinations)
    ))
  }
  ...
  nearest_matches <- event_unemployment_combinations[
    event_unemployment_combinations[,
...
#' @return Updated vecshift_data with nearest neighbor matches
#' @keywords internal
.match_events_nearest_chunked <- function(
  vecshift_data,
  events_data,
  event_name_column,
  person_id_column,
  ...
  if (progress) {
    message(sprintf(
      "Processing nearest neighbor matching for %d persons in %d chunks of size %d",
      n_persons,
      n_chunks,
  ...
  for (chunk_idx in seq_len(n_chunks)) {
    if (progress) {
      message(sprintf(
        "Processing nearest neighbor chunk %d/%d",
        chunk_idx,
        n_chunks
      ))
    }
    ...
    for (person_cf in chunk_persons) {
      ...
      event_midpoints <- (as.numeric(person_events$event_start) +
        as.numeric(person_events$event_end)) /
        2
      unemployment_midpoints <- (as.numeric(person_unemployment$inizio) +
        as.numeric(person_unemployment$fine)) /
        2
      ...
      for (j in seq_len(nrow(person_events))) {
        ...
        vecshift_data[
          .internal_idx == target_row_idx,
          (col_dist) := min_distance
        ]
        vecshift_data[
          .internal_idx == target_row_idx,
          (col_quality) := "nearest"
