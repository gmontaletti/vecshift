...
#' }
add_unemployment_periods <- function(vecshift_data, min_date = NULL, max_date = NULL, min_duration = 1L, add_head = FALSE, add_tail = TRUE) {
  ...
  required_cols <- c("cf", "inizio", "fine", "arco", "prior", "id", "over_id", "durata")
  missing_cols <- setdiff(required_cols, names(vecshift_data))
  if (length(missing_cols) > 0) {
    stop(paste("Missing required vecshift columns:", paste(missing_cols, collapse = ", ")))
  }
...
#'
add_unemployment_tail <- function(vecshift_data, max_date, min_tail_duration = 1L) {
