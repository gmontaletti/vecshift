...
# Note: test_g6r_accessibility() function would need to be implemented
# to provide automated accessibility testing scenarios.
# 
# For now, manually test accessibility by:
# 1. Using colorblind simulators on the visualization
# 2. Testing with high contrast system settings  
# 3. Verifying keyboard navigation works
# 4. Ensuring sufficient color contrast ratios

# Example of manual accessibility validation:
cat("Accessibility checklist for g6r visualizations:\n")
cat("✓ Colorblind-friendly palette used\n") 
cat("✓ High contrast mode available\n")
cat("✓ Labels clearly visible\n")
cat("✓ Interactive elements keyboard accessible\n")
```
...
server <- function(input, output, session) {
  ...
  output$stats_table <- renderTable({
    ...
    if (nrow(data) > 0) {
      data.frame(
        Transition = paste(data$from, "→", data$to),
        Count = data$weight,
...
large_pipeline_result <- process_employment_pipeline(
  large_demo_data,
  merge_columns = "company"  # Only include columns that exist in generated data
)
...
large_transitions <- analyze_employment_transitions(
  ...
  transition_variable = "company",
  # Note: Only include statistics_variables that exist in the generated data
  # statistics_variables = c("region"),  # Uncomment if region exists
...
large_scale_plot <- plot_interactive_transitions(
  ...
  layout = "force",
  min_weight_threshold = 1,  # Filter to significant transitions
...
## Choosing Between g6r and ggraph

The vecshift package provides both interactive (g6r) and static (ggraph) network visualization options:

### Use g6r when you need:
- **Interactive exploration** of employment networks
- **Real-time filtering** and data selection capabilities  
- **Shiny dashboard integration** with user controls
- **Dynamic updates** as data changes
- **Touch-friendly interfaces** for mobile/tablet use
- **Large dataset exploration** with progressive disclosure

### Use ggraph when you need:
- **Publication-ready figures** for academic papers
- **High-resolution static images** for reports and presentations
- **Complete visual control** over every aspect of the plot
- **Reproducible outputs** that render consistently
- **Vector graphics** for scaling and print media
- **Integration** with existing ggplot2 workflows

### Comparison Example

```{r comparison-demo}
# Interactive version (g6r)
...
# Static version (ggraph) 
# static_plot <- plot_transitions_network(
#   transitions_data = company_transitions,
#   layout = "fr",
#   accessibility_mode = FALSE,
#   title = "Employment Transitions Network",
#   subtitle = "Static publication-ready visualization"
# )

# Both approaches use the same underlying transition data and 
# support similar accessibility features and color palettes
```

## Cross-References to Other Vignettes

For comprehensive coverage of vecshift's visualization capabilities:

- **"Static Network Visualizations with ggraph"** - Detailed guide to publication-ready network plots
- **"Vecshift Visualization Themes"** - Color palettes, accessibility features, and theme system
- **"Status Classification"** - Employment status analysis and pattern recognition
- **"Understanding Date Logic"** - Temporal data processing fundamentals
...
- Combine static (ggraph) and interactive (g6r) approaches in comprehensive reports
